...
class RegionVisualizer(tk.Frame):
    ...
    def _validate(self):
        ...
        for r in range(N):
            ...
            if not ns or not CLUE_FUNCS[r](ns):
                row_ok.append(False)
            else:
                row_ok.append(True)
                all_nums.extend(ns)
        if len(all_nums) != len(set(all_nums)):
            # duplicate -> mark rows containing dup as invalid
            dup = {n for n in all_nums if all_nums.count(n) > 1}
            for r in range(N):
                ns = self._row_numbers(r)
                if ns and any(n in dup for n in ns):
                    row_ok[r] = False
        grand = sum(all_nums) if all(row_ok) else None
        return row_ok, grand
    ...
    def _double_action(self, x, y):
        ...
        r, c = rc
        reg = GRID[r][c]
        cur = self.state.region_digit[reg]
        val = simpledialog.askinteger("Region digit", f"Digit for region {reg} (0-clear):", minvalue=0, maxvalue=9)
        if val is None:
            return

        for r_reg, c_reg in self._get_region_cells(reg):
            for dr, dc in self.DIRS:
                nr, nc = r_reg + dr, c_reg + dc
                if self._inside_grid(nr, nc):
                    nreg = GRID[nr][nc]
                    if nreg != reg and self.state.region_digit.get(nreg, 0) == val and val != 0:
                        messagebox.showerror("Invalid Digit", f"Digit {val} clashes with adjacent region {nreg}.")
                        return
