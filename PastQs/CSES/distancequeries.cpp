...
void gen_tree(ll &n, vector<vector<ll>> &children, vector<vector<ll>> &adj) {
    ...
    FOR (i, n) {
        if (parent[i] != -1) {
            adj[i].erase(find(adj[i].begin(), adj[i].end(), parent[i]));
        }
    }
}
...
int main() {
    ...
    while (q--) {
        ...
        ll v = check_depth(lo, depths[lo], ranges, ref);
        ll rv = depth[a] - depth[v] + depth[b] - depth[v];
        cout << rv << '\n';
    }
