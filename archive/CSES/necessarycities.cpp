...
struct Bridges {
    ...
    vector<bool> is_bridge;     // output list

    explicit Bridges(ll _n, vector<vector<ll>> &_adj)
        : n(_n), adj(_adj),
          tin(n, -1), low(n, -1), vis(n, false), is_bridge(n, false) {}
    ...
    void dfs(ll v, ll p) {
        ...
        for (ll to : adj[v]) {
            ...
            if (vis[to]) {                      // back-edge
                ...
                if (low[to] >= tin[v] && p != -1) is_bridge[v] = true;
            }
        }

        if (p == -1 && dfs_children > 1) is_bridge[v] = true; // root with multiple children
    }
};
