...
int main() {
    ...
    ll n; cin >> n;
    vector<pll> cats0(n), cats;
    for (auto &[x, y] : cats0)
        cin >> x >> y;
    sort(all(cats0), [](const pll &a, const pll &b) {
    ...
    ConvexHullTrick cht;
    vector<ll> dp(n);
    cht.add_line(cats[0].x, 0);
    dp[0] = cht.query(cats[0].y);
    for (ll i = 1; i < n; i++) {
        cht.add_line(cats[i].x, dp[i - 1]);
        dp[i] = cht.query(cats[i].y);
    }
    cout << dp[n - 1] << '\n';
