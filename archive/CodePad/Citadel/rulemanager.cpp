...
private:
    ...
    unordered_map<string, SymbolData> symbolMap;

    void tryHalt(const string& ticker) {
        auto& data = symbolMap[ticker];
        if (!data.violated) return;

        bool parentsHalted = data.parents.empty() || 
            all_of(data.parents.begin(), data.parents.end(), 
                [this](const string& parent) { return symbolMap[parent].halted; });
...
public:
    void onPriceRule(const string& ticker, double priceMove) {
        auto& data = symbolMap[ticker];
        data.rule = priceMove;
        data.violated = false;
        data.halted = false;
    ...
    void onPriceUpdate(const string& ticker, double latestPrice) {
        auto& data = symbolMap[ticker];
        if (data.firstPrice == 0) {
            data.firstPrice = latestPrice;
        }
        data.latestPrice = latestPrice;

        if (abs((latestPrice - data.firstPrice) / data.firstPrice) >= data.rule) {
            data.violated = true;
            tryHalt(ticker);
        }
    }

    bool isHalted(const string& ticker) const {
        auto it = symbolMap.find(ticker);
        return it != symbolMap.end() && it->second.halted;
    }

    void addDependency(const string& parentTicker, const string& childTicker) {
        symbolMap[parentTicker].children.push_back(childTicker);
        symbolMap[childTicker].parents.push_back(parentTicker);
    }
};
