...
ll gen_tree(SuffixTree &st, vector<ll> &leafs, vector<ll> &depth, ll v, ll d) {
    depth[v] = d;
    ll rv = 0;
    FOR (i, st.alpha) {
        if (st.tree[v][i] != -1) {
...
int main() {
    ...
    while (true) {
        ...
        flip(s);
        SuffixTree st(s);
        vector<ll> leafs(st.cur_size), depth(st.cur_size);
        gen_tree(st, leafs, depth, 0, 0);
        ll rv = 0, rd = 0, rl = 1e9;
        FOR (i, st.cur_size) {
            if (leafs[i] >= m && depth[i] > rd) {
                rd = depth[i];
                rv = i;
                rl = st.hi[i];
            } elif (leafs[i] >= m && depth[i] == rd) {
                if (st.hi[i] < rl) {
                    rl = st.hi[i];
                    rv = i;
                }
